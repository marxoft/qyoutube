...
class PlaylistItemsModelPrivate;

class QYOUTUBESHARED_EXPORT PlaylistItemsModel : public Model
{
    Q_OBJECT
    ...
    Q_PROPERTY(QYouTube::PlaylistItemsRequest::Status status READ status NOTIFY statusChanged)
    Q_PROPERTY(QYouTube::PlaylistItemsRequest::Error error READ error NOTIFY statusChanged)
...
public:
    ...
    explicit PlaylistItemsModel(QObject *parent = 0);
    ...
    PlaylistItemsRequest::Status status() const;
    
    PlaylistItemsRequest::Error error() const;
...
private:        
    Q_DECLARE_PRIVATE(PlaylistItemsModel)
    Q_DISABLE_COPY(PlaylistItemsModel)
