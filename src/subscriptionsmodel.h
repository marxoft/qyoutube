...
class QYOUTUBESHARED_EXPORT SubscriptionsModel : public Model
{
    Q_OBJECT
    ...
    Q_PROPERTY(QYouTube::SubscriptionsRequest::Status status READ status NOTIFY statusChanged)
    Q_PROPERTY(QYouTube::SubscriptionsRequest::Error error READ error NOTIFY statusChanged)
...
public:
    enum Roles {
        ...
        IdRole,
        SnippetRole,
        SubscriberSnippetRole
    };
        
    explicit SubscriptionsModel(QObject *parent = 0);
    ...
    SubscriptionsRequest::Status status() const;
    
    SubscriptionsRequest::Error error() const;
...
private:        
    Q_DECLARE_PRIVATE(SubscriptionsModel)
