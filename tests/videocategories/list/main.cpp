...
int main(int argc, char *argv[]) {
    ...
    if (args.size() < 3) {
        qWarning() << "Usage: videocategories-list PART FILTERS [PARAMS]";
        return 0;
    }
    ...
    QVariantMap filters = QtJson::Json::parse(args.takeFirst()).toMap();
    QVariantMap params = args.isEmpty() ? QVariantMap() : QtJson::Json::parse(args.takeFirst()).toMap();

    QSettings settings;

    QYouTube::VideoCategoriesRequest request;
    ...
    request.list(part, filters, params);
