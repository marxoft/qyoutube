...
int main(int argc, char *argv[]) {
    ...
    if (args.size() < 3) {
        qWarning() << "Usage: playlists-update RESOURCE PART [PARAMS]";
        return 0;
    }
    ...
    QVariantMap params = QtJson::Json::parse(args.takeFirst()).toMap();

    QSettings settings;

    QYouTube::PlaylistsRequest request;
    ...
    request.update(resource, part, params);
